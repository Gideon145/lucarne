...
### Judges â€” open [`/judge`](https://lucarne-xyz.vercel.app/judge) for a wallet-free, one-page verification surface (live lifetime nonce, contract status, on-chain proofs). Build progress is broadcast in real time at [x.com/lucarne_xyz](https://x.com/lucarne_xyz).

---

## What Is Lucarne?

Lucarne is an autonomous AI agent that scores all **48 World Cup 2026 nations 0â€“100 every 60 seconds** and writes each score as a cryptographic attestation directly to **X Layer Mainnet (Chain ID 196)** â€” immutable, timestamped, verifiable by anyone with a block explorer.
...
Click any nation in the terminal to unlock a **paid AI brief** â€” generated live by Claude, gated via **x402 micropayment on the OKX Onchain OS**.
...
The 2026 FIFA World Cup will be the most-watched event in history. Billions of people will form opinions about which nations are dangerous, which are overrated, which are quietly peaking â€” and they will be reading **opinions, not signals**. Twitter accounts pivot. Sportsbook odds move silently. Pundits hedge after the fact.

**No one is forced to stand behind their call before the whistle blows.**

Lucarne fixes this with a single primitive: **on-chain attestation with economic bonding**. Every signal is computed, hashed, and written to X Layer before kickoff. Every match prediction comes with the agent's own OKB stake. There is no editing the record after the fact. Bad calls aren't buried â€” they're permanent.

---

## Live Verification â€” Click Every Link
...
The agent wallet has **over 14,000 confirmed transactions** on X Layer Mainnet â€” every one of them either a country score attestation, a match signal, a pool stake, or a settlement call. That is the proof. It cannot be faked.

---

## Pipeline Validation (Live Club-Football Stress Test, May 20â€“23 2026)

Before betting Lucarne's reputation on the 2026 World Cup, we ran the **entire pipeline end-to-end on real club matches** â€” signal generation â†’ on-chain signal lock â†’ community vote â†’ agent-bonded pool â†’ bet â†’ settlement â†’ soulbound NFT mint. Every game below served two purposes: a real public signal call, **and** the moment we shipped the next layer of the stack.

| # | Game | Date | Signal Call | Signal Tx | Result | Layer Shipped |
|---|---|---|---|---|---|---|
| 1 | **UEL Final** â€” SC Freiburg vs Aston Villa | May 20 | HOME (Aston Villa) | [`0x4e98f3c0...`](https://www.oklink.com/xlayer/tx/0x4e98f3c011ad8c512acc61805550658208370674dd82b8d3820706384aa33f65) | **Aston Villa win â€” CORRECT** Â· resolve [`0x48de5700...`](https://www.oklink.com/xlayer/tx/0x48de570075c862184c854d2ad0ca4c8ea2666808fb05e1e55310368968cd9467) | Full pipeline e2e: signal generation + on-chain attestation + result resolution |
| 2 | **Eredivisie PO** â€” Ajax vs Groningen | May 21 | HOME (Ajax) @ 99.8% | [`0x38e3c6cd...`](https://www.oklink.com/xlayer/tx/0x38e3c6cd2e8abe7833839b0840c1ab8bda5afb33ea52879483288fb9dfd62daa) | **Ajax win â€” CORRECT** Â· pending resolve | **Community voting shipped** â€” [LucarnePredictions](https://www.oklink.com/xlayer/address/0x178565919FFebC4b57ca04112d0FFFaD946Df6E7) contract, fans now vote on-chain alongside the agent |
| 3 | **Serie A** â€” Fiorentina vs Atalanta | May 22 | AWAY (Atalanta) | [`0x6983a191...`](https://www.oklink.com/xlayer/tx/0x6983a19169803ad0a03355586d289c1b644d31802ae0ae7f297eff8b50f504d5) | 0-0 draw â€” **WRONG, agent stake forfeited** | **Agent-bonded betting shipped** â€” SignalPool live; agent stake [`0x291c672c...`](https://www.oklink.com/xlayer/tx/0x291c672c3e3af5ee96ccf4e1ef5fa43258399f1876ef2a805b05e2b256b0b002), first user bet [`0x9176860e...`](https://www.oklink.com/xlayer/tx/0x9176860e7fe9c53142ef399f316fa7a988e8b8219c3c58dcd2658060c2e3da81) |
...
|---|---|---|
| **Agent stake** â€” Fiorentina/Atalanta pool opened (AWAY Â· 0.05 OKB) | [`0x291c672c...`](https://www.oklink.com/xlayer/tx/0x291c672c3e3af5ee96ccf4e1ef5fa43258399f1876ef2a805b05e2b256b0b002) | First SignalPool stake |
| **First user bet** â€” 0.01 OKB on AWAY (Fiorentina/Atalanta) | [`0x9176860e...`](https://www.oklink.com/xlayer/tx/0x9176860e7fe9c53142ef399f316fa7a988e8b8219c3c58dcd2658060c2e3da81) | Pool received its first counterparty bet |
...
| **NFT mint** â€” ICalledIt soulbound proof | [`0x01ec8778...`](https://www.oklink.com/xlayer/tx/0x01ec8778625381ff40025a73ed1534c3a2c2c27fb76eee3be35b7587fd97e2de) | First "I Called It" NFT minted |
| **NFT mint** â€” ICalledIt soulbound proof | [`0xdc7120d5...`](https://www.oklink.com/xlayer/tx/0xdc7120d57a82670e9773f09404df5f0ef0c95aedeba5083de25f566175158321) | Second mint, different game |
...
### SignalAttestor.sol â€” `0x2Dcbd501...0FAe81`
...
### MatchSignalAttestor.sol â€” `0x9693d19C...018482`
Pre-kickoff match signal lock. Records `homeProb`, `drawProb`, `awayProb` (basis points), `signalScore` (0â€“100 confidence), `signalCall` (HOME/DRAW/AWAY), and `dataHash` (provenance of input data). Called by the agent before the whistle blows.

### MatchResultAttestor.sol â€” `0x81AF1dfF...855377bF`
After full time, anyone can call `resolve(gameId, actualOutcome)`. The contract reads the prior `signalCall` from `MatchSignalAttestor` and computes `signalCorrect` on-chain â€” the verdict is permanent.

### SignalPool.sol (v2) â€” `0xEe15Dc83...12ccEa67`
...
| `reclaimNoWinner(gameId, outcome)` | If the winning bucket is empty (no counterparty), stakers reclaim their own stake â€” prevents permanent fund lock when niche outcomes hit |

### ICalledItNFT.sol (v2) â€” `0xBC2200d9...96F0adf`
...
| Component | Stack |
|---|---|
| **Chain** | X Layer Mainnet (Chain ID 196) â€” every contract, every tx |
| **Gas Token** | OKB native â€” cheap enough to attest 48 nations every 60s |
...
| **Explorer** | OKLink (`oklink.com/xlayer`) â€” every tx hash in this README resolves there |
...
â”‚           â””â”€â”€ logger.ts
â”‚
â”œâ”€â”€ contracts/               Solidity 0.8.24 Â· Hardhat v3 Â· TypeScript ESM
â”‚   â”œâ”€â”€ contracts/
...
â”‚
â”œâ”€â”€ frontend/                Next.js 14 App Router Â· Viem Â· TypeScript
â”‚   â”œâ”€â”€ app/
â”‚   â”œâ”€â”€ components/
...
â”‚
â””â”€â”€ polybot/                 FastAPI sidecar â€” Polymarket data + AI brief router
    â””â”€â”€ server.py
```
...
**2. Every claim is on-chain verifiable.** This README has 12+ tx hash links above. Every single one resolves on OKLink. Open OKX Wallet, switch to X Layer, paste any address â€” it's all there.

**3. Skin in the game.** The agent doesn't just predict â€” it stakes OKB on every match call before kickoff. When it gets one wrong (Fiorentina/Atalanta), that OKB is gone. Real money, real consequences, real signal quality discipline.

**4. World Cup timing.** Built specifically for the 2026 FIFA World Cup. 48 nations already wired into the agent loop. As group-stage matches begin, the infrastructure is already in production â€” no migration, no scramble.
...
**6. Composable primitives.** `MatchSignalAttestor` is independent of `SignalPool` is independent of `ICalledItNFT`. Any of them can be reused by other dApps. We aren't building a closed silo â€” we're shipping public goods.

---

## Track Coverage â€” X Layer X Cup
...
| **Trading** | âœ… Shipped | [`SignalPool v2`](https://www.oklink.com/xlayer/address/0xEe15Dc83cD4AcD16D8698831d468B1FE12ccEa67) parimutuel pool â€” real OKB stakes, real user bets, real settlement; agent bonds every match call |
| **NFT** | âœ… Shipped | [`ICalledItNFT v2`](https://www.oklink.com/xlayer/address/0xBC2200d99980661fef938eE72001BAaE496F0adf) soulbound ERC-721 â€” real mints [`0x01ec8778â€¦`](https://www.oklink.com/xlayer/tx/0x01ec8778625381ff40025a73ed1534c3a2c2c27fb76eee3be35b7587fd97e2de), [`0xdc7120d5â€¦`](https://www.oklink.com/xlayer/tx/0xdc7120d57a82670e9773f09404df5f0ef0c95aedeba5083de25f566175158321) |
...
| **GameFi** | âœ… Shipped | [`SurvivorPool`](https://www.oklink.com/xlayer/address/0x7250E9480A025bF59EedD271DFB88C5BC2f8c12F) â€” pick a nation each round, survive on momentum score â‰¥ 30, last survivor(s) split the pot (0.001 OKB entry). Live at [`/survivor`](https://lucarne-xyz.vercel.app/survivor) |
...
|---|---|
| **`okx-agent-payments-protocol`** (x402) | `polybot/server.py` â€” paid Claude AI briefs are gated by an x402 micropayment (`xlayer-mainnet`, USDC `0x74b7f1â€¦6d22`, `pay-to` = agent wallet). Replay-protection via per-nonce set. Hit `GET /brief/{iso3}` â†’ 402 â†’ pay â†’ 200 |
| **`okx-dapp-discovery`** (Polymarket plugin) | Polymarket gamma-api drives the per-nation probability signal for all 48 World Cup nations and every live match (`polybot/server.py` â†’ `POLYMARKET_IDS`, `WC_FIXTURES`) |
...
| **`okx-agentic-wallet`** (pattern) | Agent wallet [`0xC8D9â€¦47C3`](https://www.oklink.com/xlayer/address/0xC8D92Bfd397A7ccaaf6B44466F2951070A3947C3) operates as a self-driven agentic wallet on X Layer â€” 14,000+ mainnet tx, signal-driven write gate, no human intervention in the scoring loop |

**Why this matters for judging:** every claim in this section is grep-able. Open `polybot/server.py` and search for `x402`, `X402_NETWORK`, `xlayer-mainnet`, `LUCARNE_WALLET` â€” the wiring is real, not aspirational.

---

## Traction (as of May 23, 2026)

> Every number below is independently verifiable on X Layer Mainnet or in the live dashboard â€” no marketing, just on-chain facts.

- **14,000+ confirmed mainnet transactions** on agent wallet [`0xC8D9...47C3`](https://www.oklink.com/xlayer/address/0xC8D92Bfd397A7ccaaf6B44466F2951070A3947C3) â€” lifetime nonce, growing in real time. Live count:
  ```bash
  ...
  ```
- **Autonomous since launch** â€” 60-second cadence, persistent nonce across Railway restarts, signal-driven write gate (|Î”Score|>3, regime change, or 4h heartbeat) keeps gas burn minimal while preserving every meaningful regime transition.
- **7 contracts deployed on X Layer Mainnet** â€” SignalAttestor, MatchSignalAttestor, MatchResultAttestor, SignalPool v2 (with `reclaimNoWinner`), ICalledItNFT v2 (soulbound ERC-721), LucarnePredictions (community vote), plus legacy v1 of pool + NFT preserved for reference.
- **Full pipeline exercised on 5 real club matches** in 4 days (May 20â€“23) â€” signal generation, attestation, community voting, agent-bonded pools, real user bets, soulbound NFT mints â€” **2 correct calls, 1 real OKB loss, 2 pending**. (See Pipeline Validation table above.)
- **First user bet landed:** [`0x9176860e...`](https://www.oklink.com/xlayer/tx/0x9176860e7fe9c53142ef399f316fa7a988e8b8219c3c58dcd2658060c2e3da81) Â· **First soulbound NFT minted:** [`0x01ec8778...`](https://www.oklink.com/xlayer/tx/0x01ec8778625381ff40025a73ed1534c3a2c2c27fb76eee3be35b7587fd97e2de).
- **Continuous on Railway** (production, 24/7) â€” frontend on Vercel, Polybot sidecar on Railway, agent autonomously writing to mainnet every cycle.
- **Live dashboard:** [lucarne-xyz.vercel.app](https://lucarne-xyz.vercel.app) â€” every nation, every match, every tx hash, all clickable into OKLink.
- **Public build log:** [x.com/lucarne_xyz](https://x.com/lucarne_xyz) â€” every shipped layer broadcast in real time with the corresponding mainnet tx.

---

## For Judges â€” 60-Second Verification Path

> Every claim in this README is independently verifiable on OKLink or via a single `curl`. No setup required.

**Step 1 â€” Open Judge Mode (10s)**
...
**Step 2 â€” Confirm 14k+ mainnet TXs (10s)**
...
**Step 3 â€” Watch a fresh TX land (60s)**

Open [the agent wallet on OKLink](https://www.oklink.com/xlayer/address/0xC8D92Bfd397A7ccaaf6B44466F2951070A3947C3) and refresh. A new `attest()` transaction will appear within ~60 seconds. Click it â€” decoded input shows `country`, `score`, `regime`, and `signalHash`.

**Step 4 â€” Verify the wrong call (15s)**

We don't hide losses. Click the Fiorentina/Atalanta signal tx [`0x6983a191...`](https://www.oklink.com/xlayer/tx/0x6983a19169803ad0a03355586d289c1b644d31802ae0ae7f297eff8b50f504d5) â€” the agent staked 0.05 OKB on Atalanta. The match drew 0-0. The OKB is gone. That's signal discipline.

---

## Known Limitations (Honest Framing)

Lucarne does not fake liveness. Every degraded path is surfaced rather than papered over â€” and the 14k+ TX trail is the proof these recovery paths work in production, not in theory.

- **v1 vs v2 pools.** The four pool/NFT activity hashes above (Fio/Ata stake + user bet, Inter stake, Barca stake, both NFT mints) hit the **v1** SignalPool/NFT contracts (`0xd6E29fFcâ€¦` and the v1 NFT) because they predate the v2 redeploy that added `reclaimNoWinner`. All *new* pools from this point forward open on [`SignalPool v2`](https://www.oklink.com/xlayer/address/0xEe15Dc83cD4AcD16D8698831d468B1FE12ccEa67). We kept v1 deployed and visible for full transparency â€” every loss, every locked stake, every recovery path is on record.
- **If a SignalPool outcome has no winners** (e.g. niche AWAY win nobody bet on) â†’ `reclaimNoWinner(gameId, outcome)` lets stakers withdraw their own stake â€” v2-only behaviour, by design.
- **If RPC drops mid-cycle** â†’ ethers v6 fetches pending nonce explicitly to avoid `nonce-too-low` errors specific to X Layer's RPC behavior. The lifetime nonce never gets out of sequence.
- **If Polymarket data is stale** â†’ write-gate suppresses the attestation; nothing fake is ever signed.
- **If the agent process crashes** â†’ Railway restarts it. The agent re-reads nonce from chain and resumes â€” proven repeatedly across 14k+ TXs.
- **0G Compute / x402 mainnet endpoints for paid AI briefs** â†’ wired via Polybot sidecar; falls back to free local generation when the x402 facilitator is unreachable. No fake "premium brief" is ever served.
...
- **Not financial advice.** Country scores, match signals, and AI briefs are momentum-tracking outputs derived from public Polymarket data and football form. **Nothing in this product is a recommendation to buy, sell, hold, or stake any digital asset.** Signals are wrong all the time â€” we deliberately surfaced [the Fiorentina/Atalanta loss](https://www.oklink.com/xlayer/tx/0x6983a19169803ad0a03355586d289c1b644d31802ae0ae7f297eff8b50f504d5) as on-chain proof of that.
- **Digital assets are volatile.** OKB staked into a `SignalPool` can be lost in full. Pools settle from on-chain match results â€” once `settle()` is called, the outcome is irreversible. Stake only what you can afford to lose.
...
`gameId` is always `keccak256(slug)` â€” consistent across all contracts.
